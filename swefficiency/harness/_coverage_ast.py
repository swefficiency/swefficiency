...
from unidiff.constants import LINE_TYPE_REMOVED, LINE_TYPE_ADDED
...
argparser.add_argument("--repo_directory", type=str, default="/testbed/", help="File to get source file imports for")
argparser.add_argument("--test_files", type=str, default=None, help="Comma delimited test files of interest")
argparser.add_argument("--source_files", type=str, default=None, help="Comma delimited source files of interest")
argparser.add_argument("--outfile", type=str, help="Outfile to write relevant tests to")
argparser.add_argument("--ignore_depth", type=int, default=1, help="Depth of AST search to ignore")
argparser.add_argument("--patch_file", type=str, default="/tmp/patch.diff", help="Patch file to use")
...
class FileLineRangeTracker:
    ...
    def get_ranges(self, filename=None):
        ...
        """
        if filename is None:
            return sorted((iv.begin, iv.end, filename) for filename, tree in self.file_trees.items() for iv in tree)
...
def get_symbols_in_script(python_file):
    ...
    return python_file_script.get_names(all_scopes=True, definitions=False, references=True), identify_type_hints(python_file_text)
...
def process_file(python_file, depth=0, line_start=None, line_end=None):
    ...
    for name in all_names:        
        ...
        if any(coordinates_intersect(hint, (name_start, name_end)) for hint in type_hints):
            # Ignore type hints
            continue
        
        if coordinates_intersect((name_start, name_end), ((line_start, 0), (line_end, 0))):
            pruned_names.append(name)
        
    
    for name in pruned_names:
        try:
            dependent_names = name.goto(follow_builtin_imports=False, follow_imports=True)
        except:
            continue
        ...
        for dn in dependent_names:
            ...
            relevant_source_files[(module_file, definition_start[0], definition_end[0])] = depth
...
def process_patch_set(range_tracker: FileLineRangeTracker, patch_set):
    ...
    for patched_file in patch_set:
        ...
        lines_executed_in_covered_file = [
            ln for start, end in range_tracker.get_ranges(abs_path_patched_file) for ln in range(start, end) 
        ]
        ...
        for hunk in patched_file:
            ...
            hunk_intersects_with_coverage = lines_executed_in_covered_file.intersection(post_edit_lines_of_interest)
...
def process_test_file(test_file_tuple, patch_set=None):
    ...
    while len(files_to_explore) > 0:
        ...
        if current_file.endswith(".py"):
            _, additional_relevant_source_files = process_file(current_file, line_start=line_start, line_end=line_end, depth=depth)        
            ...
            for (dependent_file, dependent_file_start, dependent_file_end) in additional_relevant_source_files.keys():
                if FILE_IGNORE_FN(dependent_file):
                    continue
                
                temp_tree.add_range(dependent_file, dependent_file_start, dependent_file_end + 1)
                
            for interval_begin, interval_end, filename in temp_tree.get_ranges():   
                files_to_explore.append((filename, interval_begin, interval_end - 1, depth + 1))
...
with multiprocessing.Pool(processes=8) as pool:  # Adjust number of processes as needed
    for test_file, result in pool.imap_unordered(process_test_file, enumerate(test_files)):
...
sorted_tests_to_relevant_source_files = [
    test for test in sorted_tests_to_relevant_source_files if tests_to_relevant_source_files[test]
]
...
sorted_tests_to_relevant_source_files = list(set([
    *sorted_tests_to_relevant_source_files,
    *additional_possible_test_names
]))
