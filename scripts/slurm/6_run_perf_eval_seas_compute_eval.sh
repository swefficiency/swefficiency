...
PER_JOB_MEMORY="768G"  # Memory per job, adjust as needed
PARTITION="seas_compute,sapphire,intermediate"  # Partition to submit to, adjust as needed
...
#SBATCH --j "perf_eval_${PARTITION}"      # Job name
#SBATCH -o perf_eval_${repo}_${PARTITION}_%A_%a.out  # STDOUT file, %A = job ID, %a = array index
#SBATCH -e perf_eval_${repo}_${PARTITION}_%A_%a.err  # STDERR file
...
echo \$CR_PAT | docker login ghcr.io -u TODO --password-stdin
...
scripts/vm/run_eval.sh jeff_perf_ground_truth_recheck$INDEX $NUM_JOB_WORKERS preds_08012025_gemini25flash.json
EOT
