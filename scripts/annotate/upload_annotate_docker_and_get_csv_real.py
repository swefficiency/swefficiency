...
parser.add_argument(
    "--coverage_run_dir", type=Path, required=True, help="Coverage run directory."
)

args = parser.parse_args()

# Iterate over all dirs in coverage_run_dir and return dir names that have "covering_tests.txt".
covering_tests_dirs = []

for dir in args.coverage_run_dir.iterdir():
    if dir.is_dir() and (dir / "covering_tests.txt").exists():
        covering_tests_dirs.append(dir)

instance_ids = [dir.name for dir in covering_tests_dirs]
print(instance_ids)
...
def run_command(command):
    try:
        # Run cmd and print output to console.
        subprocess.run(
