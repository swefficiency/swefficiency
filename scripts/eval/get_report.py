...
def evaluate_instance(instance: dict, gold_run: Path, pred_run: Path, use_correctnes_files=True) -> dict:
    ...
    if pred_run_entry.exists():
        ...
        pred_speedup_ratio = pred_perf_info["before_mean"] / pred_perf_info["after_mean"]
    else:
    ...
    if not correctness_dir.exists():
        print(f"Instance {instance_id} has no correctness output directory.", flush=True)
        pred_speedup_ratio = 1.0
        return {
            "instance_id": instance_id,
            "pred_speedup_ratio": 1.0,
            "gold_speedup_ratio": gold_speedup_ratio,
            "human_speedup_ratio": 1.0 / gold_speedup_ratio if gold_speedup_ratio != 0 else 0,
            "correctness": 0.0,
            "correctness_pct": 0.0,
    ...
    if not use_correctnes_files:
        ...
        pred_statuses = json.loads((pred_run / instance_id / "covering_test_status.json").read_text())
    ...
    if correctness_pct < 1.0:
        print(f"Instance {instance_id} failed {len(pass_to_pass) - len(passed_tests)} out of {len(pass_to_pass)} pass-to-pass tests.", flush=True)
        for test in pass_to_pass:
    ...
    return {
        ...
        "gold_speedup_ratio": gold_speedup_ratio,
        "human_speedup_ratio": adjusted_pred_speedup_ratio / gold_speedup_ratio if gold_speedup_ratio != 0 else 0,
        "correctness": 0. if correctness_pct != 1.0 else 1.,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gold_run", type=Path, required=True, help="Path to the ground truth run dir")
    parser.add_argument("--pred_run", type=Path, required=True, help="Path to the predicted run dir")
    parser.add_argument("--num_workers", type=int, default=4, help="Number of workers for parallel processing")
    parser.add_argument("--output_dir", type=Path, default=Path("./eval_reports"), help="Output directory for the report")
