...
# Model eval reports
eval_reports = [
    ("Claude 3.7 Sonnet (OH)", "eval_reports/eval_report_us.anthropic.claude-3-7-sonnet-20250219-v1_0_maxiter_100_N_v0.51.1-no-hint-run_1.csv"),
    ("GPT-5 Mini (OH)", "eval_reports/eval_report_gpt-5-mini_maxiter_100_N_v0.51.1-no-hint-run_1.csv"),
    ("Gemini 2.5 Flash (OH)", "eval_reports/eval_report_gemini-2.5-flash_maxiter_100_N_v0.51.1-no-hint-run_1.csv"),
    ("GPT-5 Mini (SWE-agent)", "eval_reports/eval_report_default_sweperf_openai__openai--gpt-5-mini__t-1.00__p-1.00__c-1.00___swefficiency_full_test.csv"),
    ("Claude 3.7 Sonnet (SWE-agent)", "eval_reports/eval_report_default_sweperf_claude__anthropic--claude-3-7-sonnet-20250219__t-0.00__p-1.00__c-1.00___swefficiency_full_test.csv"),
    ("Gemini 2.5 Flash (SWE-agent)", "eval_reports/eval_report_default_sweperf_gemini__gemini--gemini-2.5-flash__t-0.00__p-1.00__c-1.00___swefficiency_full_test.csv"),
]
...
def _prep_model_df(report_path, sort_col):
    ...
    valid = (
        np.isfinite(df.get(sort_col, np.nan)) & (df[sort_col] > 0) &
        np.isfinite(df.get("human_speedup_ratio", np.nan)) & (df["human_speedup_ratio"] > 0)
    )
...
def _global_log_grid(model_data_list, num_points, start_quantile):
    ...
    xs = [d["x_sorted"] for d in model_data_list if d is not None and d["x_sorted"].size > 0]
    if not xs:
        return np.array([])
...
def compute_curve_logspaced(selected_eval_reports, sort_col, num_points=30, align_models=True, start_quantile=None):
    """
    ...
    global_grid = _global_log_grid(list(per_model.values()), num_points, start_quantile) if align_models else None
    ...
    for model_name, d in per_model.items():
        ...
        thresholds = global_grid if align_models else _per_model_log_grid(d, num_points, start_quantile)
...
pre_edit_xy = compute_curve_logspaced(
    eval_reports, "pre_edit_runtime",
    num_points=NUM_POINTS_LOG, align_models=ALIGN_MODELS, start_quantile=START_QUANTILE
)
gold_xy = compute_curve_logspaced(
    eval_reports, "gold_speedup_ratio",
    num_points=NUM_POINTS_LOG, align_models=ALIGN_MODELS, start_quantile=START_QUANTILE
)
patchlen_xy = compute_curve_logspaced(
    eval_reports, "patch_length",
    num_points=NUM_POINTS_LOG, align_models=ALIGN_MODELS, start_quantile=START_QUANTILE
)
